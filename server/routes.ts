...
function broadcastToClients(data: any) {
  ...
  app.post("/api/support/create-ticket", async (req, res) => {
    try {
      ...
      if (!userId) {
        return res.status(401).json({ message: "Please login to create a support ticket" });
      }

      const { subject, message, category, priority, imageData } = req.body;
      ...
      let imageUrl = null;
      if (imageData) {
        imageUrl = imageData; // Store as base64 data URL
      }
      ...
      await storage.createNotification({
        ...
        message: `Your support ticket "${subject}" has been created. We'll respond as soon as possible.`,
        type: "info"
      });
  ...
  app.get("/api/support/my-tickets", async (req, res) => {
    try {
      const userId = req.session.userId;
      if (!userId) {
        return res.status(401).json({ message: "Please login to view tickets" });
      }
  ...
  app.get("/api/support/ticket/:id", async (req, res) => {
    try {
      const userId = req.session.userId;
      if (!userId) {
        return res.status(401).json({ message: "Please login to view ticket" });
      }
  ...
  app.post("/api/support/ticket/:id/respond", async (req, res) => {
    try {
      const userId = req.session.userId;
      if (!userId) {
        return res.status(401).json({ message: "Please login to respond" });
      }
      ...
      // Create notification
      const targetUserId = isAdmin ? ticket.userId : userId;
      const notificationTitle = isAdmin ? "Support Response Received" : "Support Message Sent";
      const notificationMessage = isAdmin 
        ? `You received a response to your support ticket: "${ticket.subject}"`
        : `Your response has been sent for ticket: "${ticket.subject}"`;
  ...
  app.put("/api/admin/support-tickets/:id/status", async (req, res) => {
    try {
      ...
      const updatedTicket = await storage.updateSupportTicketStatus(ticketId, status, userId);
